...
let o='The '
const n=r,s=e=>`"${e}"`
switch(t=t.map((e=>Array.isArray(e)?e.map(s).join(' or '):s(e))),n){case 1:o+=`${t[0]} argument`
break
...
return void 0===t?r:`${r} (${t})`})),n('ERR_OUT_OF_RANGE',((t,r,o,n,s=false)=>{if(!o)throw new e.ERR_MISSING_ARGS('range')
let a=s?r:`The value of "${r}" is out of range.`,R
if(Number.isInteger(n)&&Math.abs(n)>2**32)R=i(String(n))
else if('bigint'===typeof n){if(R=String(n),n>2n**32n||n<-(2n**32n))R=i(R)
R+='n'}else R=n
return a+=` It must be ${o}. Received ${R}`,a}),o),n('ERR_INVALID_SYNC_FORK_INPUT',((e,t)=>`Asynchronous forks do not support Buffer, TypedArray, DataView or string input: ${t}`),t),n('ERR_UNKNOWN_SIGNAL',((e,t)=>`Unknown signal: ${t}`),t),n('ERR_MODULE_NOT_FOUND',((e,t)=>`Cannot find module '${t}'`)),n('ERR_INVALID_PACKAGE_CONFIG',((e,t,r,o)=>`Invalid package config ${t}${r?` while importing ${r}`:''}${o?`. ${o}`:''}`)),n('ERR_PACKAGE_IMPORT_NOT_DEFINED',((e,t,r,o)=>`Package import specifier "${t}" is not defined${r?` in package ${r}package.json`:''} imported from ${o}`),t),n('ERR_PACKAGE_PATH_NOT_EXPORTED',((e,t,r,o=void 0)=>{if('.'===r)return`No "exports" main defined in ${t}package.json${o?` imported from ${o}`:''}`
...
return o+=`. Received protocol '${t.protocol}'`,o})),n('ERR_INVALID_PACKAGE_TARGET',((e,t,r,o,n=false,s=void 0)=>{const i='string'===typeof o&&!n&&o.length&&!o.startsWith('./')
if('.'===r){if(false!==n)throw Error(`isImport must be false when key = '.'`)
return`Invalid "exports" main target ${JSON.stringify(o)} defined `+`in the package config ${t}package.json${s?` imported from ${s}`:''}${i?'; targets must start with "./"':''}`}return`Invalid "${n?'imports':'exports'}" target ${JSON.stringify(o)} defined for '${r}' in the package config ${t}package.json${s?` imported from ${s}`:''}${i?'; targets must start with "./"':''}`})),n('ERR_INVALID_MODULE_SPECIFIER',((e,t,r,o=void 0)=>`Invalid module "${t}" ${r}${o?` imported from ${o}`:''}`),t),n('ERR_UNKNOWN_FILE_EXTENSION',((e,t,r,o)=>{let n=`Unknown file extension "${t}" for ${r}`
if(o)n+=`. ${o}`
return n}),t)
const s=(e,t,r)=>{const o=new Error(`${t} (${e}): ${r}`)
return o.errno=e,o.code=e.toString(),o.syscall=t,o}
class AbortError extends Error{constructor(e='The operation was aborted'){super(e),this.code='ABORT_ERR',this.name='AbortError'}}const i=e=>{let t='',r=e.length,o='-'===e[0]?1:0
for(;r>=o+4;r-=3)t=`_${e.slice(r-3,r)}${t}`
return`${e.slice(0,r)}${t}`}
export{AbortError,e as codes,s as errnoException}
