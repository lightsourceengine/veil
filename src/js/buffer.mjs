...
Buffer.isBuffer = (arg) => arg instanceof Buffer;
...
const from = (value, encoding, length) => {
  const arrayBuffer = native.fromArrayBuffer(value, encoding, length);

  if (arrayBuffer) {
    return arrayBuffer;
  }

  if (Buffer.isBuffer(value) || typeof value === 'string' || Array.isArray(value)) {
