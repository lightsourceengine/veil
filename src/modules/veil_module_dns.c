...
static void after_get_addr_info(uv_getaddrinfo_t* req, int status,
  ...
  if (status == 0 && res != NULL) {
    ...
    if (err) {
      ...
      args[argc++] = jerry_null();
    }

    args[argc++] = jerry_string_sz(ip);
    args[argc++] = jerry_number(family);
  } else {
    args[argc++] = iotjs_jval_create_error_without_error_flag(
...
jerry_value_t veil_init_dns(void) {
  jerry_value_t dns = jerry_object();
