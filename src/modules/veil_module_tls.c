...
static iotjs_tls_context_t *iotjs_tls_context_create(
  ...
  jerry_object_set_native_ptr(jobject, &tls_context_native_info, tls_context);
...
static iotjs_tls_t *iotjs_tls_create(const jerry_value_t jobject,
  ...
  jerry_object_set_native_ptr(jobject, &this_module_native_info, tls_data);
...
JS_FUNCTION(tls_init) {
  ...
  iotjs_tls_context_t *tls_context = jerry_object_get_native_ptr(jtls_context, &tls_context_native_info);
  if (!tls_context) {
...
static void iotjs_tls_notify_error(iotjs_tls_t *tls_data) {
  jerry_value_t jerror = jerry_string_sz("error");
  jerry_value_t jmessage = jerry_string_sz("TLS error");
  ...
  jerry_value_free(fn);
  jerry_value_free(jargv[0]);
  jerry_value_free(jargv[1]);
}
...
static void tls_handshake(iotjs_tls_t *tls_data, jerry_value_t jthis) {
  ...
  jerry_value_t jargv[2] = { jerry_boolean(error),
                             jerry_boolean(authorized) };
  ...
  do {
    ...
    if (tls_data->state != TLS_CONNECTED) {
      IOTJS_ASSERT(tls_data->state == TLS_HANDSHAKE_READY ||
                   tls_data->state == TLS_HANDSHAKE_IN_PROGRESS);
      tls_handshake(tls_data, call_info_p->this_value);
    ...
    while (true) {
      ...
      if (ret_val == 0) {
        ...
        jerry_value_t fn =
            iotjs_jval_get_property(call_info_p->this_value, IOTJS_MAGIC_STRING_ONREAD);
        iotjs_invoke_callback(fn, call_info_p->this_value, &jbuffer, 1);
...
jerry_value_t veil_init_tls(void) {
  jerry_value_t jtls = jerry_object();
