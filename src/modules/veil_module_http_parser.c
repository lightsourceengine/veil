...
typedef struct {
  ...
  cstr fields[HEADER_MAX];
  cstr values[HEADER_MAX];
  size_t n_fields;
  size_t n_values;
...
static void iotjs_http_parserwrap_initialize(
  ...
  http_parserwrap->url = cstr_init();
  http_parserwrap->status_msg = cstr_init();
  http_parserwrap->n_fields = 0;
  http_parserwrap->n_values = 0;
  http_parserwrap->flushed = false;
  http_parserwrap->cur_jbuf = jerry_null();
  http_parserwrap->cur_buf = NULL;
  http_parserwrap->cur_buf_len = 0;

  for (int32_t i = 0; i < HEADER_MAX; i++) {
    http_parserwrap->fields[i] = cstr_init();
    http_parserwrap->values[i] = cstr_init();
  }
}
...
static void iotjs_http_parserwrap_create(const jerry_value_t jparser,
  ...
  jerry_object_set_native_ptr(jparser, &this_module_native_info, http_parserwrap);
...
static void iotjs_http_parserwrap_destroy(void *native_p, struct jerry_object_native_info_t *info_p) {
  ...
  for (size_t i = 0; i < HEADER_MAX; i++) {
    cstr_drop(&http_parserwrap->fields[i]);
    cstr_drop(&http_parserwrap->values[i]);
  }
...
static jerry_value_t iotjs_http_parserwrap_make_header(
    iotjs_http_parserwrap_t* http_parserwrap) {
  jerry_value_t jheader = jerry_array(http_parserwrap->n_values * 2);
...
static int iotjs_http_parserwrap_on_message_begin(http_parser* parser) {
  ...
  cstr_clear(&http_parserwrap->url);
  cstr_clear(&http_parserwrap->status_msg);
  return 0;
}
...
static int iotjs_http_parserwrap_on_url(http_parser* parser, const char* at,
  ...
  iotjs_http_parserwrap_t* http_parserwrap =
      (iotjs_http_parserwrap_t*)(parser->data);
  cstr_append_n(&http_parserwrap->url, at, length);
  return 0;
}
...
static int iotjs_http_parserwrap_on_status(http_parser* parser, const char* at,
  ...
  iotjs_http_parserwrap_t* http_parserwrap =
      (iotjs_http_parserwrap_t*)(parser->data);
  cstr_append_n(&http_parserwrap->status_msg, at, length);
  return 0;
}
...
static int iotjs_http_parserwrap_on_header_field(http_parser* parser,
  ...
  if (http_parserwrap->n_fields == http_parserwrap->n_values) {
    ...
    cstr_clear(&http_parserwrap->fields[http_parserwrap->n_fields - 1]);
  }
  IOTJS_ASSERT(http_parserwrap->n_fields == http_parserwrap->n_values + 1);
  cstr_append_n(&http_parserwrap->fields[http_parserwrap->n_fields - 1],
                      at, length);
...
static int iotjs_http_parserwrap_on_header_value(http_parser* parser,
  ...
  if (http_parserwrap->n_fields != http_parserwrap->n_values) {
    ...
    cstr_clear(&http_parserwrap->values[http_parserwrap->n_values - 1]);
  }

  IOTJS_ASSERT(http_parserwrap->n_fields == http_parserwrap->n_values);

  cstr_append_n(&http_parserwrap->values[http_parserwrap->n_values - 1],
                      at, length);
...
static int iotjs_http_parserwrap_on_headers_complete(http_parser* parser) {
  ...
  if (http_parserwrap->flushed) {
    ...
    jerry_value_free(jheader);
    if (http_parserwrap->parser.type == HTTP_REQUEST) {
      IOTJS_ASSERT(!cstr_empty(http_parserwrap->url));
...
static int iotjs_http_parserwrap_on_body(http_parser* parser, const char* at,
  ...
  jerry_value_t argv[3] = { http_parserwrap->cur_jbuf,
                            jerry_number(at - http_parserwrap->cur_buf),
                            jerry_number(length) };

  iotjs_invoke_callback(func, jobj, argv, 3);

  jerry_value_free(argv[1]);
  jerry_value_free(argv[2]);
  jerry_value_free(func);
...
JS_FUNCTION(js_func_execute) {
  ...
  iotjs_http_parserwrap_set_buf(parser, jerry_null(), NULL, 0);
  ...
  if (!nativeparser->upgrade && nparsed != buf_len) {
    ...
    return jerry_number(nparsed);
  }
}


static jerry_value_t iotjs_http_parser_pause(const jerry_call_info_t *call_info_p, int paused) {
  JS_DECLARE_THIS_PTR(http_parserwrap, parser);
...
static void http_parser_register_methods_object(jerry_value_t target) {
  jerry_value_t methods = jerry_array(26);
  jerry_value_t method_name;
  jerry_value_t res;
  uint32_t idx = 0;
#define V(num, name, string)                                       \
  method_name = jerry_string_sz(#string); \
  res = jerry_object_set_index(methods, idx++, method_name);        \
  jerry_value_free(res); \
...
#undef V
  ...
  jerry_value_free(methods);
}

jerry_value_t veil_init_http_parser(void) {
  jerry_value_t http_parser = jerry_object();

  jerry_value_t jparser_cons = jerry_function_external(http_parser_cons);
