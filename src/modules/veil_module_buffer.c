...
iotjs_bufferwrap_t* iotjs_bufferwrap_create(const jerry_value_t jobject,
  ...
  jerry_object_set_native_ptr(jobject, &this_module_native_info, bufferwrap);

  bufferwrap->length = length;

  IOTJS_ASSERT(jerry_object_get_native_ptr(jobject, &this_module_native_info));
  return bufferwrap;
}
...
iotjs_bufferwrap_t* iotjs_bufferwrap_from_jbuffer(const jerry_value_t jbuffer) {
  ...
  void* buffer = jerry_object_get_native_ptr(jbuffer, &this_module_native_info);
...
size_t iotjs_bufferwrap_length(iotjs_bufferwrap_t* bufferwrap) {
  ...
  size_t length = (size_t)iotjs_jval_as_number(jlength);
  IOTJS_ASSERT(length == bufferwrap->length);
  jerry_value_free(jlength);
#endif
...
jerry_value_t iotjs_bufferwrap_create_buffer(size_t len) {
  jerry_value_t jres_buffer = jerry_object();
...
JS_FUNCTION(buffer_write_decode) {
  ...
  return jerry_number((double)(nbytes - 1));
}
...
JS_FUNCTION(buffer_read_uint8) {
  ...
  return jerry_number((uint8_t)buffer[offset]);
}
...
static jerry_value_t to_hex_string(const uint8_t* data, size_t length) {
  if (length == 0) {
    return jerry_string(NULL, 0, JERRY_ENCODING_UTF8);
  }
  ...
  jerry_value_t ret_value = jerry_string(str, buffer_length, JERRY_ENCODING_UTF8);
  IOTJS_RELEASE(str);
...
static jerry_value_t to_base64_string(const uint8_t* data, size_t length) {
  ...
  jerry_value_t ret_value = jerry_string(buffer, buffer_length, JERRY_ENCODING_UTF8);
...
JS_FUNCTION(buffer_to_string) {
  ...
  if (!jerry_validate_string((const jerry_char_t*)data, length, JERRY_ENCODING_UTF8)) {
    return JS_CREATE_ERROR(TYPE, "Invalid UTF-8 string");
  }

  return jerry_string((const jerry_char_t*)data, length, JERRY_ENCODING_UTF8);
}
...
JS_FUNCTION(buffer_byte_length) {
  ...
  jerry_size_t size = jerry_string_size(jargv[0], JERRY_ENCODING_UTF8);
  return jerry_number(size);
}
...
JS_FUNCTION(buffer_from_array_buffer) {
  ...
  jerry_length_t length = jerry_arraybuffer_size(jargv[0]);
  ...
  if (jargc >= 2) {
    ...
    double offset_value = jerry_value_as_number(offset_num);
    if (isnan(offset_value)) {
      offset_value = 0;
    }
    jerry_value_free(offset_num);
  ...
  if (jargc >= 3) {
    ...
    if (jerry_value_is_number(jargv[2])) {
      ...
      length_value = jerry_value_as_number(jargv[2]);
...
jerry_value_t veil_init_buffer(void) {
  jerry_value_t buffer = jerry_function_external(buffer_constructor);
