...
napi_status napi_create_array(napi_env env, napi_value* result) {
  NAPI_TRY_ENV(env);
  JERRYX_CREATE(jval, jerry_array(0));
...
napi_status napi_create_array_with_length(napi_env env, size_t length,
                                          napi_value* result) {
  NAPI_TRY_ENV(env);
  JERRYX_CREATE(jval, jerry_array(length));
  return napi_assign_nvalue(jval, result);
}
...
napi_status napi_create_arraybuffer(napi_env env, size_t byte_length,
  ...
  JERRYX_CREATE(jval, jerry_arraybuffer(byte_length));
  uint8_t* data_ptr = jerry_arraybuffer_data(jval);
...
#undef CASE_NAPI_TYPEDARRAY_TYPE
  ...
  JERRYX_CREATE(jval,
                jerry_typedarray_with_buffer_span(jtype,
                                                           jval_arraybuffer,
                                                           byte_offset,
                                                           length));
  NAPI_ASSIGN(result, AS_NAPI_VALUE(jval));
...
napi_status napi_create_dataview(napi_env env, size_t byte_length,
  ...
  JERRYX_CREATE(jval_dv, jerry_dataview(jval_arraybuffer, byte_offset,
                                               byte_length));
...
static const jerry_object_native_info_t napi_external_native_info = {
  .free_cb = (jerry_object_native_free_cb_t)napi_external_destroy
};
...
napi_status napi_create_external(napi_env env, void* data,
  ...
  jerry_object_set_native_ptr(AS_JERRY_VALUE(nval), &napi_external_native_info, info);
...
napi_status napi_create_object(napi_env env, napi_value* result) {
  NAPI_TRY_ENV(env);
  JERRYX_CREATE(jval, jerry_object());
  return napi_assign_nvalue(jval, result);
}
...
static napi_status napi_create_error_helper(jerry_error_t jerry_error_type,
  ...
  jerry_size_t msg_size = jerry_string_size(jval_msg, JERRY_ENCODING_UTF8);
  jerry_char_t* raw_msg = IOTJS_CALLOC(msg_size + 1, jerry_char_t);
  jerry_size_t written_size =
      jerry_string_to_buffer(jval_msg, JERRY_ENCODING_UTF8, raw_msg, msg_size);
  NAPI_WEAK_ASSERT(napi_invalid_arg, written_size == msg_size);
  raw_msg[msg_size] = '\0';

  jerry_value_t jval_error = jerry_throw_sz(jerry_error_type, (const char*)raw_msg);
  ...
  if (code != NULL) {
    ...
    jval_error = jerry_exception_value(jval_error, true);
...
static napi_status napi_number_convert_from_c_type_helper(napi_env env,
                                                          ...
                                                          napi_value* result) {
  NAPI_TRY_ENV(env);
  JERRYX_CREATE(jval, jerry_number(value));
  return napi_assign_nvalue(jval, result);
}
...
napi_status napi_get_value_double(napi_env env, napi_value value,
  ...
  NAPI_ASSIGN(result, jerry_value_as_number(jval));
  NAPI_RETURN(napi_ok);
}
...
#define DEF_NAPI_NUMBER_CONVERT_FROM_NVALUE(type, name)             \
  napi_status napi_get_value_##name(napi_env env, napi_value value, \
    ...
    jerry_value_t jval = AS_JERRY_VALUE(value);                     \
    NAPI_TRY_TYPE(number, jval);                                    \
    double num = jerry_value_as_number(jval);                      \
...
napi_status napi_create_symbol(napi_env env, napi_value description,
  ...
  JERRYX_CREATE(jval, jerry_symbol(AS_JERRY_VALUE(description)));
  NAPI_ASSIGN(result, AS_NAPI_VALUE(jval));
  NAPI_RETURN(napi_ok);
}
...
napi_status napi_create_string_utf8(napi_env env, const char* str,
  ...
  JERRYX_CREATE(jval,
                jerry_string((jerry_char_t*)str, length, JERRY_ENCODING_UTF8));
  return napi_assign_nvalue(jval, result);
}
...
napi_status napi_get_array_length(napi_env env, napi_value value,
  ...
  NAPI_TRY_ENV(env);
  jerry_value_t jval = AS_JERRY_VALUE(value);
  NAPI_ASSIGN(result, jerry_array_length(jval));
  NAPI_RETURN(napi_ok);
}
...
napi_status napi_get_prototype(napi_env env, napi_value object,
  ...
  JERRYX_CREATE(jval_proto, jerry_object_proto(jval));
...
napi_status napi_get_value_bool(napi_env env, napi_value value, bool* result) {
  ...
  return napi_assign_bool(jerry_value_to_boolean(jval), result);
}

napi_status napi_get_boolean(napi_env env, bool value, napi_value* result) {
  NAPI_TRY_ENV(env);
  JERRYX_CREATE(jval, jerry_boolean(value));
  return napi_assign_nvalue(jval, result);
}
...
napi_status napi_get_dataview_info(napi_env env, napi_value dataview,
  ...
  if (!jerry_feature_enabled(JERRY_FEATURE_DATAVIEW)) {
    NAPI_ASSIGN(byte_length, 0);
    NAPI_ASSIGN(data, NULL);
    NAPI_ASSIGN(arraybuffer, AS_NAPI_VALUE(jerry_undefined()));
  ...
  JERRYX_CREATE(jval_arraybuffer,
                jerry_dataview_buffer(jval, (jerry_length_t*)byte_offset,
                                          (jerry_length_t*)byte_length));
  uint8_t* ptr = jerry_arraybuffer_data(jval_arraybuffer);
...
napi_status napi_get_typedarray_info(napi_env env, napi_value typedarray,
  ...
  jerry_typedarray_type_t jtype = jerry_typedarray_type(jval);
...
#undef CASE_JERRY_TYPEDARRAY_TYPE
  ...
  jerry_length_t jlength = jerry_typedarray_length(jval);
  jerry_length_t jbyte_offset;
  jerry_length_t jbyte_length;
  JERRYX_CREATE(jval_arraybuffer,
                jerry_typedarray_buffer(jval, &jbyte_offset,
                                            &jbyte_length));
...
napi_status napi_get_value_string_utf8(napi_env env, napi_value value,
  ...
  size_t str_size = jerry_string_size(jval, JERRY_ENCODING_UTF8);
  if (buf == NULL) {
  ...
  jerry_size_t written_size =
      jerry_string_to_buffer(jval, JERRY_ENCODING_UTF8, (jerry_char_t*)buf, bufsize);
...
NAPI_EXTERN napi_status napi_get_value_string_latin1(napi_env env,
  ...
  if (buf == NULL) {
    NAPI_ASSIGN(result, (size_t)jerry_string_length(jval));
    NAPI_RETURN(napi_ok);
  }
...
napi_status napi_get_null(napi_env env, napi_value* result) {
  NAPI_TRY_ENV(env);
  JERRYX_CREATE(jval, jerry_null());
  return napi_assign_nvalue(jval, result);
}

napi_status napi_get_undefined(napi_env env, napi_value* result) {
  NAPI_TRY_ENV(env);
  JERRYX_CREATE(jval, jerry_undefined());
  return napi_assign_nvalue(jval, result);
}
...
napi_status napi_coerce_to_bool(napi_env env, napi_value value,
  ...
  JERRYX_CREATE(jval_result, jerry_boolean(res));
...
napi_status napi_typeof(napi_env env, napi_value value,
  ...
  NAPI_TRY_ENV(env);
  jerry_value_t jval = AS_JERRY_VALUE(value);
  jerry_type_t type = jerry_value_type(jval);
  ...
  switch (type) {
    ...
    case JERRY_TYPE_OBJECT: {
      if (jerry_object_get_native_ptr(jval, &napi_external_native_info)) {
...
napi_status napi_instanceof(napi_env env, napi_value object,
  ...
  jerry_value_t is_instance =
      jerry_binary_op(JERRY_BIN_OP_INSTANCEOF, jval_object, jval_cons);
  if (jerry_value_is_error(is_instance)) {
    jerry_value_free(is_instance);
    NAPI_ASSIGN(result, false);
  } else {
    NAPI_ASSIGN(result, jerry_value_to_boolean(is_instance));
  }
...
napi_status napi_strict_equals(napi_env env, napi_value lhs, napi_value rhs,
  ...
  jerry_value_t is_equal =
      jerry_binary_op(JERRY_BIN_OP_STRICT_EQUAL, jval_lhs, jval_rhs);
  if (jerry_value_is_error(is_equal)) {
    jerry_value_free(is_equal);
    NAPI_RETURN(napi_generic_failure);
  }

  return napi_assign_bool(jerry_value_to_boolean(is_equal), result);
}
...
napi_status napi_create_promise(napi_env env, napi_deferred* deferred,
  ...
  jerry_value_t jpromise = jerry_promise();
...
napi_status napi_resolve_deferred(napi_env env, napi_deferred deferred,
  ...
  jerry_value_t res = jerry_promise_resolve(promise, AS_JERRY_VALUE(resolution));
...
napi_status napi_reject_deferred(napi_env env, napi_deferred deferred,
  ...
  jerry_value_t promise = AS_JERRY_VALUE(*((napi_value*)deferred));
  jerry_value_t res =
      jerry_promise_reject(promise, AS_JERRY_VALUE(rejection));
  jerry_value_free(promise);
  free(deferred);
  if (jerry_value_is_error(res)) {
    NAPI_INTERNAL_CALL(napi_throw(env, AS_NAPI_VALUE(res)));
